...
namespace Com.DanLiris.Service.Purchasing.WebApi.Controllers.v1.GarmentDeliveryOrderNonPOControllers
{
    [Produces("application/json")]
    [ApiVersion("1.0")]
    [Route("v{version:apiVersion}/garment-delivery-orders-non-po")]
    [Authorize]
    public class GarmentDeliveryOrderNonPOController : Controller
    {
        private string ApiVersion = "1.0.0";
        public readonly IServiceProvider serviceProvider;
        private readonly IMapper mapper;
        private readonly IGarmentDeliveryOrderNonPOFacade facade;
        private readonly IdentityService identityService;

        public GarmentDeliveryOrderNonPOController(IServiceProvider serviceProvider, IMapper mapper, IGarmentDeliveryOrderNonPOFacade facade)
        {
        ...
        public IActionResult GetForCustoms(string Keyword = "", string Filter = "{}", string BillNo = null)
        {
            ...
            var newData = mapper.Map<List<GarmentDeliveryOrderNonPOViewModel>>(Data);
            Dictionary<string, object> Result =
                   new ResultFormatter(ApiVersion, General.OK_STATUS_CODE, General.OK_MESSAGE)
                   .Ok(newData);
            return Ok(Result);
        ...
        public IActionResult Get(int id)
        {
            try
            {
                ...
                var viewModel = mapper.Map<GarmentDeliveryOrderNonPOViewModel>(model);
                if (viewModel == null)
                {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] GarmentDeliveryOrderNonPOViewModel ViewModel)
        {
            try
            {
                identityService.Username = User.Claims.Single(p => p.Type.Equals("username")).Value;
                ...
                var model = mapper.Map<GarmentDeliveryOrderNonPO>(ViewModel);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] GarmentDeliveryOrderNonPOViewModel ViewModel)
        {
            try
            {
                identityService.Username = User.Claims.Single(p => p.Type.Equals("username")).Value;
