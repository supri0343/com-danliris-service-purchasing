...
namespace Com.DanLiris.Service.Purchasing.WebApi.Controllers.v1.Report
{
    ...
    public class LocalPurchasingBookReportController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Get(string no, int accountingUnitId, int accountingCategoryId, DateTime? dateFrom, DateTime? dateTo, DateTime? inputDate, bool isValas, int divisionId)
        {
            try
            {
                //var data = await localPurchasingBookReportFacade.GetReport(no, unit, category, dateFrom, dateTo, isValas);
                var data = await localPurchasingBookReportFacade.GetReportV2(no, accountingUnitId, accountingCategoryId, dateFrom, dateTo, inputDate, isValas, divisionId);
        ...
        [HttpGet("pdf")]
        public async Task<IActionResult> GetPdf(string no, int accountingUnitId, int accountingCategoryId, DateTime? dateFrom, DateTime? dateTo, DateTime? inputDate, bool isValas, int divisionId)
        {
            try
            {
        ...
        [HttpGet("download")]
        public async Task<IActionResult> GetXls(string no, int accountingUnitId, int accountingCategoryId, DateTime? dateFrom, DateTime? dateTo, DateTime? inputDate, bool isValas, int divisionId)
        {
            try
            {
                byte[] xlsInBytes;
                ...
                var xls = await localPurchasingBookReportFacade.GenerateExcel(no, accountingUnitId, accountingCategoryId, dateFrom, dateTo, inputDate, isValas, divisionId);
