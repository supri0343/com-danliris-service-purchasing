    ...
    public class GarmentInvoiceViewModel : BaseViewModel, IValidatableObject
    {
		...
		public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
		{
			...
			}else
			{
				...
				if (purchasingDbContext.GarmentInvoices.AsNoTracking().ToList().Where(DO => DO.InvoiceNo.Equals(invoiceNo) && DO.Id != Id && DO.InvoiceDate.ToOffset((new TimeSpan(7, 0, 0))) == invoiceDate && DO.SupplierId == supplier.Id).Count() > 0)
				{
