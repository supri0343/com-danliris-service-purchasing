        ...
        public List<GarmentStockReportViewModel> GetStockQuery(string ctg, string unitcode, DateTime? datefrom, DateTime? dateto, int offset)
        {
            ...
            var BalanceStock = (from a in dbContext.GarmentStockOpnames
                                ...
                                select new GarmentStockReportViewModelTemp
                                {
                                    ...
                                    RO = b.RO
                                }).ToList().GroupBy(x => new { x.BeginningBalanceUom, x.Buyer, x.EndingUom, x.PlanPo, x.ExpandUom, x.NoArticle, x.PaymentMethod, x.ProductCode, x.ReceiptUom, x.RO }, (key, group) => new GarmentStockReportViewModelTemp
                                {
            ...
            var SATerima = (from a in dbContext.GarmentUnitReceiptNoteItems
                            ...
                            select new GarmentStockReportViewModelTemp
                            {
                                ...
                                RO = a.RONo
                            }).ToList().GroupBy(x => new { x.BeginningBalanceUom, x.Buyer, x.EndingUom, x.ExpandUom, x.NoArticle, x.PaymentMethod, x.PlanPo, x.ProductCode, /*x.ProductName,*/ x.ReceiptUom, x.RO }, (key, group) => new GarmentStockReportViewModelTemp
                            {
                                BeginningBalanceQty = Math.Round(group.Sum(x => x.BeginningBalanceQty), 2, MidpointRounding.AwayFromZero),
                                BeginningBalanceUom = key.BeginningBalanceUom,
                                Buyer = key.Buyer,
