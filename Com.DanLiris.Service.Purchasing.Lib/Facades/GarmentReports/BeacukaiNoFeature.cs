...
namespace Com.DanLiris.Service.Purchasing.Lib.Facades.GarmentReports
{
    public class BeacukaiNoFeature : IBeacukaiNoFeature
    {
        ...
        public async Task<List<BeacukaiNoFeatureViewModel>> GetBeacukaiNo(string filter, string keyword)
        {
            ...
            var Query1 = Query.Distinct().GroupBy(x => new { x.BCType, x.BCDate, x.ProductCode, x.PO, x.BCNo, x.DONo, x.RONo, x.Bruto,x.Netto,x.URNNo,x.URNDate,x.UENNo,x.UENDate,x.UENType,x.SupplierName }, (key, group) => new BeacukaiNoFeatureViewModel
            {
                BCType = key.BCType,
                BCDate = key.BCDate,
