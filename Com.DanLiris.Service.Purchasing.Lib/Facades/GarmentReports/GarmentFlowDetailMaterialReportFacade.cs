...
namespace Com.DanLiris.Service.Purchasing.Lib.Facades.GarmentReports
{
	public class GarmentFlowDetailMaterialReportFacade : IGarmentFlowDetailMaterialReport
	{
			...
			if(unit == "SMP1")
			{
				var Query = (from a in dbContext.GarmentUnitExpenditureNoteItems
							 ...
							 select new GarmentFlowDetailMaterialViewModel
							 {
                                 ...
                                 ExpenditureCreateDate = b.CreatedUtc.AddHours(7).Date,
                                 Colour = a.Colour,
                                 Rack = a.Rack,
                                 Box = a.Box,
                                 Level = a.Level,
        ...
        public MemoryStream GenerateExcelForMII(string category, string productcode, string categoryname, string unit, string unitname, DateTimeOffset? dateFrom, DateTimeOffset? dateTo, DateTimeOffset? dateFromCreate, DateTimeOffset? dateToCreate, int offset)
        {
            var Query = GetQuery(category, productcode, unit, dateFrom, dateTo, dateFromCreate, dateToCreate, offset);
            Query = Query.OrderByDescending(b => b.CreatedUtc);
            DataTable result = new DataTable();
            ...
            else
            {
                ...
                foreach (var item in Query)
                {
                    ...
                    index++;
                    string tanggal = item.ExpenditureDate.Value.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("id-ID"));
                    string tanggalInput = item.ExpenditureCreateDate.Value.AddHours(offset).ToString("MM/dd/yyyy", new CultureInfo("id-ID"));
                    result.Rows.Add(index, item.ProductCode, item.ProductName, item.POSerialNumber, item.ProductRemark, item.RONo,
                        item.Article, item.BuyerCode, item.RONoDO, item.ArticleDO, item.UnitDestination, item.UENNo, tanggal, item.Colour, item.Rack, item.Box, item.Level, item.Area, NumberFormat(item.Quantity),
                        item.UomUnit, tanggalInput);
                }
            }
            ExcelPackage package = new ExcelPackage();
            //DateTime DateFrom = dateFrom == null ? new DateTime(1970, 1, 1) : (DateTime)dateFrom.Value.DateTime;
