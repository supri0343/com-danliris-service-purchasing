...
namespace Com.DanLiris.Service.Purchasing.Lib.Facades.UnitReceiptNoteFacade
{
    public interface IUnitReceiptNoteFacade
    {
        ...
        Task<ReadResponse<UnitReceiptNoteReportViewModel>> GetReport(string urnNo, string prNo, string unitId, string categoryId, string supplierId, string divisioId, DateTime? dateFrom, DateTime? dateTo, int page, int size, string Order, int offset);
        Task<MemoryStream> GenerateExcel(string urnNo, string prNo, string unitId, string categoryId, string supplierId, string divisionId, DateTime? dateFrom, DateTime? dateTo, int offset);
