...
namespace Com.DanLiris.Service.Purchasing.Test.Facades.UnitPaymentOrderTests
{
    public class BasicTest
    {
        ...
        public async Task Should_Success_Get_Report_All()
        {
            UnitPaymentOrderFacade facade = new UnitPaymentOrderFacade(GetServiceProvider(GetCurrentMethod()).Object, _dbContext(GetCurrentMethod()));
            var model = await _dataUtil(facade, GetCurrentMethod()).GetTestData();
            var Response = facade.GetReportAll(null, model.SupplierId,model.UPONo, DateTime.MinValue, DateTime.MaxValue, It.IsAny<DateTime>(), It.IsAny<string>(), 1, 25, "{}", 7);
            Assert.NotEmpty(Response.Item1);
        }
        ...
        public async Task Should_Success_Get_Report_All_Null_Parameter()
        {
            UnitPaymentOrderFacade facade = new UnitPaymentOrderFacade(GetServiceProvider(GetCurrentMethod()).Object, _dbContext(GetCurrentMethod()));
            var model = await _dataUtil(facade, GetCurrentMethod()).GetTestData();
            var Response = facade.GetReportAll("", "","", DateTime.MinValue, DateTime.MaxValue, It.IsAny<DateTime>(), It.IsAny<string>(), 1, 25, "{}", 7);
            Assert.NotEmpty(Response.Item1);
        }
        ...
        public async Task Should_Success_Get_Generate_Data_Excel()
        {
            UnitPaymentOrderFacade facade = new UnitPaymentOrderFacade(GetServiceProvider(GetCurrentMethod()).Object, _dbContext(GetCurrentMethod()));
            var model = await _dataUtil(facade, GetCurrentMethod()).GetTestData();
            var Response = facade.GenerateExcel(null, model.SupplierId, model.UPONo, DateTime.MinValue, DateTime.MaxValue, It.IsAny<DateTime>(), It.IsAny<string>(), 7);
        ...
        public async Task Should_Success_Get_Generate_Data_Excel_Null_Parameter()
        {
            UnitPaymentOrderFacade facade = new UnitPaymentOrderFacade(GetServiceProvider(GetCurrentMethod()).Object, _dbContext(GetCurrentMethod()));
            var model = await _dataUtil(facade, GetCurrentMethod()).GetTestData();
            var Response = facade.GenerateExcel("", "","", DateTime.MinValue, DateTime.MaxValue, It.IsAny<DateTime>(), It.IsAny<string>(), 7);
            Assert.IsType<System.IO.MemoryStream>(Response);
        }
        #endregion
