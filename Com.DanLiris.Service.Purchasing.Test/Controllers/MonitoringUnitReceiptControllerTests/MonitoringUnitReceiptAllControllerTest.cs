...
namespace Com.DanLiris.Service.Purchasing.Test.Controllers.MonitoringUnitReceiptControllerTests
{
	...
	public class MonitoringUnitReceiptAllControllerTest
	{
        ...
        private MonitoringUnitReceiptAllDeleted ViewModel
        {
            get
            {
                return new MonitoringUnitReceiptAllDeleted
                {
                    no = "no"
                };
            }
...
//------------Menu Baru Monitoring History Delet Data BUM Controller------//
       ...
       public void Should_Sucess_Get_Deleted_Report_Data()
        {
            var mockFacade = new Mock<IMonitoringUnitReceiptAllFacade>();
            mockFacade.Setup(x => x.GetDeleteReport(null, null, null))
            ...
            var claims = new Claim[]
          {
            ...
            controller.ControllerContext = new ControllerContext()
           {
	        ...
	        controller.ControllerContext.HttpContext.Request.Headers["x-timezone-offset"] = "0";
            var response = controller.GetDelete(null, null, null);
			//Assert.Equal((int)HttpStatusCode.OK, GetStatusCode(response));
			Assert.NotNull(response);
       }

        [Fact]
        public void Should_Error_Get_Deleted_Report_Data()
        {
            var mockFacade = new Mock<IMonitoringUnitReceiptAllFacade>();
            mockFacade.Setup(x => x.GetDeleteReport("b", DateTime.Now, DateTime.Now))
                .Returns(Tuple.Create(new List<MonitoringUnitReceiptAllDeleted> { ViewModel }, 25));
